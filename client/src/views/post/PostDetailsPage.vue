    ...
    <template v-if="post">
      ...
      <div class="flex justify-between items-center">
        <div class="space-y-2">
          ...
          <h2 class="text-sm flex flex-col gap-1">
        ...
        <div class="flex gap-2 items-center">
          <button v-if="post.status === 'PUBLISH'" class="text-white bg-yellow-500" @click="draftPost">draft</button>
          <button v-else class="text-white bg-green-500" @click="publishPost">publish</button>
          <button v-else class="text-white bg-blue-500" @click="publishPost">
            <PencilIcon class="size-4" />
            <span>Edit</span>
          </button>
          <button class="text-white bg-red-500" @click="deletePost">delete</button>
        </div>
      </div>
      <div class="bg-white p-4 rounded-lg">{{ post.content }}</div>
    </template>
  </div>
</template>
